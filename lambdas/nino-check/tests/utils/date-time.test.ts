import { toEpochSecondsFromNow } from "../../src/utils/date-time";
...
describe("DateTime utils", () => {
  describe("toEpochSeconds", () => {
    ...
    it.each([
      ...
      "should return value in seconds when unit (%s) is specified and value is not specified",
      (unit, expected) => {
    ...
    it.each([
      ...
      "should return value in seconds when unit (%s) is specified for value of %s",
      (unit, value, expected) => {
        expect(toEpochSecondsFromNow(value, unit)).toBe(expected);
      }
    );

    it.each([null, "invalidUnit"])(
      "should error when time unit %s specified is not found",
      (unit) =>
        expect(() => toEpochSecondsFromNow(0, unit as TimeUnits)).toThrow(
          `Time unit must be valid: ${unit}`
        )
    );
  });
});
