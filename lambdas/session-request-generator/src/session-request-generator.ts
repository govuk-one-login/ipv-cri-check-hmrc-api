...
import { stackOutputs } from "./cloudformation-helper";
...
export class SessionRequestGenerator implements LambdaInterface {
  public async handler(
    ...
    try {
      ...
      logger.info("Exit SessionRequestGenerator");
      return {
        statusCode: 302,
        headers: {
          Location:
            frontendStackOutput.FrontUrl + (process.env.frontendEndpoint || ""),
...
async function init() {
  ...
  if (!coreInfraStackOutput) {
    ...
    coreInfraStackOutput = (await stackOutputs(
      "core-infrastructure"
    )) as Partial<{
      CriDecryptionKey1Id: string;
      CriVcSigningKey1Id: string;
  ...
  if (!apiStackOutput) {
    ...
    apiStackOutput = (await stackOutputs(apiStackName)) as Partial<{
      PrivateApiGatewayId: string;
    }>;
    sessionEndpointUrl = `https://${apiStackOutput.PrivateApiGatewayId}.execute-api.eu-west-2.amazonaws.com/${process.env.environment}/session`;
  }
  if (!frontendStackOutput) {
    logger.info("Fetching stack output for " + frontendStackName);
    frontendStackOutput = (await stackOutputs(frontendStackName)) as Partial<{
      FrontUrl: string;
    }>;
  }
