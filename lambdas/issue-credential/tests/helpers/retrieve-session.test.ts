...
describe("retrieveSession()", () => {
  ...
  it("returns as expected for some valid input", async () => {
    const result = await retrieveSession(sessionTableName, mockDynamoClient, mockAccessToken);
  ...
  it("handles a missing record correctly", async () => {
    ...
    try {
      await retrieveSession(sessionTableName, mockDynamoClient, mockAccessToken);
    } catch (error) {
      thrown = true;
